...
export default async function SuperAdminPage() {
  ...
  const activeClinics = subsData?.filter(s => s.status === 'active').length || 0;
  const suspendedClinics = subsData?.filter(s => s.status === 'suspended').length || 0;

  const { data: profiles } = await supabase.from('profiles').select('role');
  const doctorCount = profiles?.filter(p => p.role === 'doctor').length || 0;
  const assistantCount = profiles?.filter(p => p.role === 'assistant').length || 0;
