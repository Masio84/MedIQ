...
import { Layout, Check, X, ChevronUp, ChevronDown, Type, Users, Stethoscope, ClipboardList, PenTool, Hash } from 'lucide-react';
...
export default function BlockEditor({ block }: { block: TemplateBlock }) {
  const { updateBlockConfig, toggleBlock } = usePrescriptionStore();
      ...
      <div className="p-3 space-y-3">
        {block.type === 'header' && (
          <>
            ...
            <div className="space-y-1">
              <label className="text-[10px] text-gray-400 uppercase font-bold ml-1">Contacto</label>
              <textarea 
        ...
        {block.type === 'diagnosis' && (
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-gray-500 font-bold uppercase">Mostrar CIE-10</span>
            <button 
              onClick={() => handleConfigChange('showCIE10', !block.contentConfig.showCIE10)}
              className={`w-8 h-4 rounded-full relative transition-colors ${block.contentConfig.showCIE10 ? 'bg-blue-500' : 'bg-gray-300'}`}
            >
              <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${block.contentConfig.showCIE10 ? 'left-4.5' : 'left-0.5'}`} />
            </button>
          </div>
        )}
        ...
        {block.type === 'signature' && (
          <div className="space-y-4">
                ...
                {['left', 'center', 'right'].map((align) => (
                  <button
                    ...
                    className={`flex-1 py-1 text-[9px] font-bold uppercase rounded transition-all ${
                      (block.contentConfig.alignment || 'center') === align 
            ...
            <div className="flex items-center justify-between px-1">
               <span className="text-[10px] text-gray-400 uppercase font-bold">Mostrar Datos del Médico</span>
               <button 
                onClick={() => handleConfigChange('showName', !block.contentConfig.showName)}
                className={`w-8 h-4 rounded-full relative transition-colors ${block.contentConfig.showName !== false ? 'bg-blue-500' : 'bg-gray-300'}`}
               >
                 <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${block.contentConfig.showName !== false ? 'translate-x-4' : 'translate-x-0.5'}`} />
               </button>
            </div>

            <div className="flex items-center justify-between px-1">
               <span className="text-[10px] text-gray-400 uppercase font-bold">Mostrar Sello</span>
               <button 
        ...
        {block.type === 'footer' && (
          <div className="space-y-1">
            <label className="text-[10px] text-gray-400 uppercase font-bold ml-1">Texto Legal / Pie</label>
            <textarea 
        ...
        {['treatment', 'notes'].includes(block.type) && (
          <div className="py-2 text-center border-2 border-dashed border-gray-200 rounded text-gray-400">
             <span className="text-[10px] uppercase font-bold">Sin configuración extra</span>
          </div>
        )}
      </div>
    </div>
